...
不要在 Dashboard 手工创建通配策略。运行 `web/drizzle/migrations/0025_local_first_sync_and_mcp.sql`；它按表结构分别处理直接 `user_id` 和通过父表归属的记录，并安装幂等同步 RPC、tombstone 与 MCP 客户端授权表。
...
2. 执行 `DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres pnpm --filter daypage-web db:migrate`，按 journal 运行 `0000` 至 `0025`。
